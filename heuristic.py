...
class Multiobjective_heuristic(ElementwiseProblem):
...
class Minimize():
    ...
    """
    def __init__(self, 
                problem : Multiobjective_heuristic, 
                population_size : int, 
    ...
    def _create_mixed_variables(self):
        ...
        _mask_continuous = np.array(["int" for _ in range(self._problem.num_continuous_vars)])
        ...
        _crossover = MixedVariableCrossover(_masks, {
            "bin": get_crossover("bin_hux", prob=0.3),
            "int": get_crossover("int_sbx", prob=0.3),
