...
class Parameters():
    ...
    def __init__(self, G: Graph, seed : int = 0) -> None:
        ...
        _random_increasing = [np.random.randint(low=l,high=h) for l,h in [(100000,200000),(200000,400000),(400000,600000)]]
    ...
    def _create_opening_costs(self) -> np.ndarray:
        ...
        _opening_cost_landfill = lambda x, _: x * _LANDFILL_COST_PER_M_SQRD
        # Estimate for equipment cost (Conveyor belt, Air seperation system, etc.)
        EQUIPMENT_COST = [250000, 500000, 750000]
        _opening_cost_sorting = lambda x, ix: (x * _LANDFILL_COST_PER_M_SQRD)+EQUIPMENT_COST[ix]
        # Giubiasco incinerator cost: 40 Million CHF in 2004 (Inflation adjusted through: https://lik-app.bfs.admin.ch/en/lik/rechner : 42’050’290 CHF) for an area of 40,000 m^2
        _opening_cost_incinerator = lambda x, _: x * (42050290/40000)
        _opening_cost_list = np.array([function(self._direct_land_usage[ix], ix) for ix,function in enumerate([_opening_cost_sorting,_opening_cost_incinerator,_opening_cost_landfill])]).tolist()
